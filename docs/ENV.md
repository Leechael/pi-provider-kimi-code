...
| `KIMI_SHARE_DIR` | Override the `kimi-code` share directory used to look up an existing OAuth credential at `<dir>/credentials/kimi-code.json`. Default: `~/.kimi`. Mirrors upstream `kimi-code`. Read-only — this extension never writes to that file. |
...
| `KIMI_BASE_URL`            | Alias for `KIMI_CODE_BASE_URL`. Accepted for compatibility with upstream `kimi-code`, which uses this name. `KIMI_CODE_BASE_URL` wins if both are set.                                                                                                                                                                                                                                                                                                 |
| `KIMI_CODE_OAUTH_HOST`     | Override the OAuth host.                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `KIMI_OAUTH_HOST`          | Fallback OAuth host override for compatibility.                                                                                                                                                                                                                                                                                                                                                                                                        |
| `KIMI_CODE_PROTOCOL`       | Select the wire protocol. Supported values: `openai` (default), `anthropic`, or `responses`. `openai` → `openai-completions` via `/coding/v1/chat/completions`; `anthropic` → `anthropic-messages` via `/coding/v1/messages`; `responses` → `openai-responses` via `/coding/v1/responses`. Any other value is treated as `openai`; an explicit per-model protocol returned by `/models` takes precedence. `/models` reports Responses as `"response"`. |
| `KIMI_CODE_CUSTOM_HEADERS` | Newline-separated `Name: Value` headers added to model, file, usage, search, fetch, and datasource requests. Identity, authorization, content negotiation, and `X-Msh-*` names are reserved and ignored here.                                                                                                                                                                                                                                          |
| `KIMI_DATASOURCE_API_URL`  | Override the datasource tools endpoint. Defaults to `${KIMI_CODE_BASE_URL}/tools` after normal `/v1` base URL derivation.                                                                                                                                                                                                                                                                                                                              |
...
These mirror the same-name environment variables in upstream `kimi-code`. When set they apply _after_ server-side discovery, so a user-provided override always wins over `/v1/models` metadata.
